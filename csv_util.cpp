...
/*
Chu-Hsuan Lin
- update read_image_data_csv function for reading header in the csv file
- add append_header_csv function to add header to csv file
*/
 ...
 */
int read_image_data_csv( char *filename, std::vector<char *> &filenames, std::vector<std::vector<float>> &data, std::vector<char *> &header, int echo_file ) {
    FILE *fp;
    float fval;
    char img_file[256],tmp[256];

    std::vector<char*> feature_name;
    fp = fopen(filename, "r");
    if( !fp ) {
    ...
    // update for read header file
    for(;;){
        int e = getstring( fp, tmp );

        char *fname = new char[strlen(tmp) +1];
        strcpy(fname, tmp);
...
int append_header_csv(char *filename, char *col_name, std::vector<char*> &feature_name, int reset_file){
    ...
    for(int i=0;i<feature_name.size();i++) {
        ...
        char t[8];
        strcpy(t, ",");
        strcpy(tmp, feature_name[i]);
        strcat(t, tmp);
        //sprintf(tmp, ",%.4f", image_data[i] );
        std::fwrite(t, sizeof(char), strlen(t), fp );
    }
...
int read_test( char *filename, std::vector<char *> &filenames, std::vector<std::vector<float>> &data, int echo_file ){
    ...
    */
    for(;;){
        if(getstring(fp, word)){
            break;
        };

        std::cout << word << std::endl;
        //featurename.push_back(word);
